        ...
        setInterval(() => {
            if (this.hitpoints > 0) {
                if (this.isSpotted) {
                    ...
                    const distanceToCharacter = this.x - world.character.x;

                    if (Math.abs(distanceToCharacter) <= this.maxMoveDistance) {
                        ...
                        if (Math.abs(this.x - this.originalX) >= this.maxMoveDistance) {
                            this.speed *= -1;
                        }
                    }
                } else {
                    // Wenn der Charakter nicht gespottet wird, kehre zum Ausgangspunkt zurück
                    const distanceToOriginalX = this.originalX - this.x;
